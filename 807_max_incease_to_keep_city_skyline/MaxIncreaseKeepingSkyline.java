package _807maxinceasetokeepcityskyline;
...
public class MaxIncreaseKeepingSkyline {
  ...
  private static int maxIncreaseKeepingSkyline(int[][] grid) {
    ...
    for (int value : leftRight) {
      if (maxLeftRight < value) {
        maxLeftRight = value;
      }
    }
    for (int value : topBottom) {
      if (maxTopBottom < value) {
        maxTopBottom = value;
      }
    }
    System.out.println(
      "The skyline viewed from top or bottom is: " + Arrays.toString(topBottom)
    );
    System.out.println(
      "The skyline viewed from left or right is: " + Arrays.toString(leftRight)
    );
    for (int i = 0; i < grid.length; i++) {
      for (int j = 0; j < grid[0].length; j++) {
